...
public class InfoBar extends VBox {
    ...
    public InfoBar(GameMap gameMap) {
        ...
        Label citiesLbl = new Label("Cities");
        Label ministersLbl = new Label("Ministers");
        Label technologiesLbl = new Label("Technologies");
        Label neighborsLbl = new Label("Neighbors");
        Label logsLbl = new Label("Logs");
        ...
        troopSlider.valueProperty().addListener((ov, oldVal, newVal) -> {
            lbSlider.setText("Enter number of troops(" + newVal.intValue() + ")");
        });

        Pane buttonGroup = createActionButtonGroup();

        this.getChildren().addAll(infoGroup, citiesLbl, cityListView, ministersLbl, ministerListView, technologiesLbl, technologyListView, neighborsLbl, neighborListView, lbSlider, troopSlider, buttonGroup, logsLbl, logListView);
    ...
    private void onSelectedCityChangeHandler(City city) {
        ...
        troopSlider.setMax((double) city.getTroops());
    }
    ...
    private void actionButtonHandler(int actionNumber) {
        ...
        City selNeigh = neighborListView.getSelectionModel().getSelectedItem();
        Minister selMin = ministerListView.getSelectionModel().getSelectedItem();
        Technology selTech = technologyListView.getSelectionModel().getSelectedItem();

        double troopVal = troopSlider.getValue();

        gameActionHandler.handle(actionNumber, selMin, selCity, selNeigh, selTech, (int) troopVal);
    }
    ...
    public void setDisableButtons(boolean isDisabled) {
        ...
        for (int i = 0; i < this.buttons.size(); i++) {
            this.buttons.get(i).setDisable(isDisabled);
        }
