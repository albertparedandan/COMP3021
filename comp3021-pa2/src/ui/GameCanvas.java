...
public class GameCanvas extends StackPane {
    ...
    private void drawCity(City city, Image flag) {
        ...
        Image icon = city.getImage(CELL_WIDTH, CELL_HEIGHT);
        Cell loc = gameMap.getCityLocation(city);
        loc = toCanvasCoordinates(loc);

        canvasLayers[ICON_LAYER].getGraphicsContext2D().drawImage(icon, loc.getX(), loc.getY(), CELL_WIDTH, CELL_HEIGHT);
        canvasLayers[TEXT_LAYER].getGraphicsContext2D().strokeText(city.getName(), loc.getX() + CELL_WIDTH / 4, loc.getY());
        canvasLayers[FLAG_LAYER].getGraphicsContext2D().drawImage(flag, loc.getX(), loc.getY());
    }
    ...
    private void drawLinesBetweenCities(City a, City b) {
        ...
        Cell locA = gameMap.getCityLocation(a);
        locA = toCanvasCoordinates(locA);

        Cell locB = gameMap.getCityLocation(b);
        locB = toCanvasCoordinates(locB);

        if (gameMap.getCityOwner(a).equals(gameMap.getCityOwner(b))) {
            canvasLayers[LINE_LAYER].getGraphicsContext2D().setLineWidth(3);
            canvasLayers[LINE_LAYER].getGraphicsContext2D().setStroke(Color.LIGHTGREEN);
            canvasLayers[LINE_LAYER].getGraphicsContext2D().strokeLine(locA.getX() + (CELL_WIDTH / 2), locA.getY() + (CELL_HEIGHT / 2), locB.getX() + (CELL_WIDTH / 2), locB.getY()+ (CELL_HEIGHT / 2));
        }
        else {
            canvasLayers[LINE_LAYER].getGraphicsContext2D().setLineWidth(3);
